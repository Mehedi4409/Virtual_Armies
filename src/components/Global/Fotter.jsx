...
const Footer = () => {
  ...
  return (
    <footer className="relative min-h-[100dvh] w-full bg-[#050505] text-white flex flex-col justify-between antialiased overflow-hidden">
        ...
        <Wrapper>
          <div className="flex justify-between items-center border-b border-white/5 pb-8">
            <div className="flex items-center gap-3">
               ...
               <h2 className="text-[clamp(12px,1.2vw,14px)] font-bold tracking-[0.2em] uppercase text-zinc-400">Virtual Armies</h2>
            </div>
            
            <button 
              onClick={scrollToTop}
              className="group flex items-center gap-2 text-[11px] uppercase tracking-[0.1em] text-zinc-500 hover:text-white transition-colors"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 w-full gap-16 lg:gap-4 items-start">
          ...
          <div className="lg:col-span-4 flex flex-col justify-center space-y-10">
            ...
            >
              <h2 className="text-[clamp(3.5rem,8vw,6rem)] font-black leading-[0.9] tracking-tighter text-white">
                LET'S <br />
                <span className="text-zinc-900">BUILD IT.</span>
              </h2>
            </motion.div>
            
            <div className="max-w-xs space-y-8">
               <p className="text-[clamp(1rem,1.3vw,1.1rem)] text-zinc-500 font-normal leading-relaxed">
          ...
          <div className="lg:col-span-8 grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-12">
            ...
            <div className="space-y-7">
              <h4 className="text-[13px] font-medium text-[#48a9a6]">Company</h4>
              <ul className="flex flex-col gap-4">
                {links.company.map((item) => (
                  <li key={item.label}>
                    <Link to={item.path} className="text-[clamp(14px,1.4vw,15px)] text-zinc-400 hover:text-white transition-colors">
                      {item.label}
                    </Link>
                  </li>
                ))}
            ...
            {/* Column 2: Service */}
            <div className="space-y-7">
              <h4 className="text-[13px] font-medium text-[#48a9a6]">Service</h4>
              <ul className="flex flex-col gap-4">
            ...
            {/* Column 3: Sol_list_1 */}
            <div className="space-y-7">
              <h4 className="text-[13px] font-medium text-[#48a9a6]">Solutions</h4>
              <ul className="flex flex-col gap-4">
           ...
           <div className="flex flex-col md:flex-row justify-between items-center gap-10 border-t border-white/5 pt-10">
              <div className="flex gap-10">
                {links.social.map((social) => (
                   <a key={social.label} href={social.url} className="text-zinc-600 hover:text-[#48a9a6] transition-colors group flex items-center gap-2">
                      <social.icon size={18} />
              ...
              <p className="text-[11px] uppercase tracking-[0.2em] text-zinc-700">
                © {new Date().getFullYear()} Virtual Armies Inc. <span className="mx-2 text-zinc-900">/</span> Lean Infrastructure
              </p>
           </div>
        </Wrapper>
      </div>
      ...
      <div className="absolute bottom-[-2vw] left-0 w-full overflow-hidden pointer-events-none z-0 select-none opacity-[0.03] grayscale">
